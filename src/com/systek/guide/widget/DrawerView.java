...
import com.systek.guide.activity.HomeActivity;
	...
	private void initView() {
		...
		switch_mode_btn.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				SharedPreferences settings = activity.getSharedPreferences("setting", 0);
				SharedPreferences.Editor editor = settings.edit();
				if(isChecked){
					auto_mode_text.setText(activity.getResources().getString(R.string.action_close_auto_mode));
					editor.putString(Const.GUIDE_MODEL_KEY, Const.GUIDE_MODEL_AUTO);
					editor.commit();
					MyApplication.guideModel=MyApplication.GUIDE_MODEL_AUTO;
					LogUtil.i("tag", "当前模式为+++GUIDE_MODEL_AUTO");
					Intent intent =new Intent();
					intent.setAction(DescribeActivity.BeaconNotify_open);
					activity.sendBroadcast(intent);

				}else{
					auto_mode_text.setText(activity.getResources().getString(R.string.action_open_auto_mode));
					editor.putString(Const.GUIDE_MODEL_KEY, Const.GUIDE_MODEL_HAND);
					editor.commit();
					MyApplication.guideModel=MyApplication.GUIDE_MODEL_HAND;
					LogUtil.i("tag", "当前模式为+++GUIDE_MODEL_HAND");

					Intent intent =new Intent();
					intent.setAction(DescribeActivity.BeaconNotify_close);
					activity.sendBroadcast(intent);
				}
				LogUtil.i("测试数据模式", "guideModel----"+MyApplication.guideModel);
			}
		});
